...
```
        ...
        └──────┬───────┘
               │
        ┌──────┴───────┐
        │  Tree Search  │  (MCTS / BFTS / BeamSearch)
        │  Method       │
        └──────┬───────┘
               │
     ┌─────────┼─────────┐
...
| **Methods** | Search strategy (MCTS, BFTS, BeamSearch) | `src/treethink/methods/` |
...
- Methods auto-convert (MCTS → AsyncMCTS, BFTS → AsyncBFTS, etc.)
...
| Isabelle | Custom client | 🧪 Experimental |
