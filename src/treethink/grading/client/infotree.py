        ...
        if "node" in item:
            ...
            while transformed:
                ...
                for edge_obj in child_list:
                    ...
                    if (
                        not include_failed_pp
                        and child_pp == "<failed to pretty print>"
                    ):
...
def _flatten_chain(
    nodes: dict, edges: list[tuple], parent_id: int, child_id: int
):
    """
    ...
    edges[:] = [
        e for e in edges if not (e[0] == parent_id and e[1] == child_id)
    ]
...
def adjust_intervals(intervals: list[dict]) -> list[dict]:
    ...
    for iv in intervals_sorted:
        if (iv["finish_line"], iv["finish_col"]) > (
            max_finish_line,
            max_finish_col,
        ):
            max_finish_line, max_finish_col = (
                iv["finish_line"],
                iv["finish_col"],
            )
...
def retrieve_tactics(
    intervals: list[dict], source_lines: list[str]
) -> list[dict]:
    """
...
def transfer_trailing_whitespaces_and_comments(intervals: list[dict]):
    ...
    for i in range(len(intervals) - 1):
        ...
        nxt["tactic"] = (
            trailing_ws_2 + trailing_comment + trailing_ws_1 + nxt["tactic"]
        )
