...
async def process_batch(
    batch, client: Lean4Client, timeout, infotree_type, semaphore
):
...
def batch_verify_proof(
    ...
    num_proc: int = 8,
    batch_size: int = 8,
    infotree_type: Optional[str] = None,
):
    """Verify multiple proofs in batches using the Lean4 server.

    Args:
        client (KiminaClient): The KiminaClient client instance to use for
            verification.
        samples (List[dict]): List of samples to verify. Each sample must be a dictionary
            ...
            NOTE(burak): it is expected that samples is a list of dicts with
            'code' and 'custom_id' keys, though only 'code' is used in this
            function because kimina_client.check expects list of code strings.
        timeout (int, optional): Timeout in seconds for each batch.
            Defaults to 60.
        num_proc (int, optional): Number of concurrent processes. Defaults to 8.
        batch_size (int, optional): Number of samples in each batch.
            Defaults to 8.
        infotree_type (str, optional): Type of info tree to use. It is
            automatically converted to new Infotree type in kimina client.
            Defaults to None.
    ...
    response = client.check(
        ...
        infotree=Infotree(infotree_type) if infotree_type else None,
    )

    # New kimina client: response is a pydantic model
